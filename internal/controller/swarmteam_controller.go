		...
		Spec: kubeswarmv1alpha1.SwarmAgentSpec{
			...
			Infrastructure: func() *kubeswarmv1alpha1.AgentInfrastructure {
				infra := &kubeswarmv1alpha1.AgentInfrastructure{
					...
					RegistryRef: func() *corev1.LocalObjectReference {
						if team.Spec.RegistryRef == "" {
							return nil
						}
						return &corev1.LocalObjectReference{Name: team.Spec.RegistryRef}
					}(),
				}
...
func (r *SwarmTeamReconciler) isTriggerTemplate(ctx context.Context, team *kubeswarmv1alpha1.SwarmTeam) (bool, error) {
	var eventList kubeswarmv1alpha1.SwarmEventList
...
func (r *SwarmTeamReconciler) applyRoleScale(
	...
	if currentReplicas != desired {
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(team, nil, corev1.EventTypeNormal, "AutoscaleAgent", "Scaled",
	...
	for i := range runs {
		for _, step := range runs[i].Status.Steps {
			...
			case kubeswarmv1alpha1.PipelineStepPhaseRunning,
				kubeswarmv1alpha1.PipelineStepPhaseWarmingUp,
				kubeswarmv1alpha1.PipelineStepPhaseValidating:
				counts[step.Name]++
			}
		}
	}
