...
func Complete(ctx context.Context, model, systemPrompt, userMsg string, maxTokens int) (string, error) {
	...
	c, ok := p.(Completer)
	if !ok {
		return "", agenterrors.NewConfigError(
			agenterrors.ErrConfigInvalid,
			fmt.Sprintf("provider %q does not implement Completer", providerName),
			nil,
		)
	}
...
func New(name string) (LLMProvider, error) {
	if name == "" {
		return nil, agenterrors.NewConfigError(
			...
			"provider name is empty; set AGENT_PROVIDER or use Detect(model)",
			nil,
		)
	}
	...
	if !ok {
		return nil, agenterrors.NewConfigError(
			agenterrors.ErrConfigInvalid,
			fmt.Sprintf("unsupported provider %q; import its package to register it", name),
			nil,
		)
	}
	return factory(), nil
