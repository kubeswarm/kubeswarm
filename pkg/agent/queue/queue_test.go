...
// ---------------------------------------------------------------------------
// TokenUsage.ThinkingTokens (RFC-0033 phase 7)
// ---------------------------------------------------------------------------

func TestTokenUsage_ThinkingTokensField(t *testing.T) {
	// Zero value defaults to 0.
	var zero queue.TokenUsage
	if zero.ThinkingTokens != 0 {
		t.Errorf("default ThinkingTokens = %d, want 0", zero.ThinkingTokens)
	}

	// Accepts positive values and is independent of Input/OutputTokens.
	u := queue.TokenUsage{
		InputTokens:    100,
		OutputTokens:   200,
		ThinkingTokens: 321,
	}
	if u.ThinkingTokens != 321 {
		t.Errorf("ThinkingTokens = %d, want 321", u.ThinkingTokens)
	}
	if u.InputTokens != 100 {
		t.Errorf("InputTokens = %d, want 100 (must be independent of ThinkingTokens)", u.InputTokens)
	}
	if u.OutputTokens != 200 {
		t.Errorf("OutputTokens = %d, want 200 (must be independent of ThinkingTokens, not additive)", u.OutputTokens)
	}
}
