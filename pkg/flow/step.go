...
func InitializeRunSteps(f *kubeswarmv1alpha1.SwarmRun) {
	...
	f.Status.Steps = make([]kubeswarmv1alpha1.PipelineStepStatus, len(f.Spec.Pipeline))
	for i, step := range f.Spec.Pipeline {
		f.Status.Steps[i] = kubeswarmv1alpha1.PipelineStepStatus{
			Name:  step.Role,
...
func InitializeRouteStep(f *kubeswarmv1alpha1.SwarmRun) {
	...
	f.Status.Steps = []kubeswarmv1alpha1.PipelineStepStatus{
		{Name: "route", Phase: kubeswarmv1alpha1.PipelineStepPhasePending},
	}
...
func ParseRunOutputJSON(
	...
	for name, st := range statusByName {
		...
		if st.Phase != kubeswarmv1alpha1.PipelineStepPhaseSucceeded || st.Output == "" || st.OutputJSON != "" {
			continue
		}
...
func EvaluateRunLoops(
	...
	for _, step := range f.Spec.Pipeline {
		...
		if st == nil || st.Phase != kubeswarmv1alpha1.PipelineStepPhaseSucceeded {
			continue
		}
...
func ApplyCompressionResult(st *kubeswarmv1alpha1.PipelineStepStatus, compressed string) {
