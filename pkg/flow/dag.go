...
func BuildRunStatusByName(f *kubeswarmv1alpha1.SwarmRun) map[string]*kubeswarmv1alpha1.PipelineStepStatus {
	m := make(map[string]*kubeswarmv1alpha1.PipelineStepStatus, len(f.Status.Steps))
...
func DepsSucceeded(deps []string, statusByName map[string]*kubeswarmv1alpha1.PipelineStepStatus) bool {
	for _, dep := range deps {
		st, ok := statusByName[dep]
		if !ok {
			return false
		}
		if st.Phase != kubeswarmv1alpha1.PipelineStepPhaseSucceeded &&
