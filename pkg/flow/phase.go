...
func EnforceRunTimeout(f *kubeswarmv1alpha1.SwarmRun, now metav1.Time) bool {
	...
	for i, st := range f.Status.Steps {
		if st.Phase == kubeswarmv1alpha1.PipelineStepPhaseRunning ||
			...
			f.Status.Steps[i].Phase = kubeswarmv1alpha1.PipelineStepPhaseFailed
...
func UpdateRunPipelinePhase(f *kubeswarmv1alpha1.SwarmRun, templateData map[string]any) {
	...
	for _, st := range f.Status.Steps {
		...
		case kubeswarmv1alpha1.PipelineStepPhaseFailed:
			failed = true
		case kubeswarmv1alpha1.PipelineStepPhaseSucceeded, kubeswarmv1alpha1.PipelineStepPhaseSkipped:
