...
// providerMetrics is a lazily-initialised AgentMetrics instance shared by all
// Provider invocations. We cache via sync.Once so the provider does not need a
// plumb-through refactor to receive metrics from its caller.
var (
	providerMetricsOnce sync.Once
	providerMetrics     *observability.AgentMetrics
)

func getAgentMetrics() *observability.AgentMetrics {
	providerMetricsOnce.Do(func() {
		m, err := observability.NewAgentMetrics()
		if err != nil {
			return
		}
		providerMetrics = m
...
func (p *Provider) RunTask(
	...
	if apiKey == "" {
		return "", queue.TokenUsage{}, agenterrors.NewConfigError(agenterrors.ErrConfigMissing, "ANTHROPIC_API_KEY is not set; required for the anthropic provider", nil)
	}
	...
	for {
		params := buildAnthropicParams(cfg, messages, anthropicTools)
		...
		if chunkFn != nil {
			...
			llmSpan.SetAttributes(
				...
				attribute.Int64("llm.thinking_tokens", turnUsage.ThinkingTokens),
			)
			// Partial usage accumulation (DD8): accumulate any tokens reported before
			// the error so callers observe the partial cost on mid-stream failures.
			usage.InputTokens += turnUsage.InputTokens
			usage.OutputTokens += turnUsage.OutputTokens
			usage.ThinkingTokens += turnUsage.ThinkingTokens
			if err != nil {
			...
			llmSpan.End()
			if reasoningEnabled && metricsInst != nil {
				metricsInst.RecordReasoningCall(ctx, turnUsage.ThinkingTokens, reasoningAttrs...)
			}
		...
		thinkingTokens := countThinkingTokens(resp.Content)
		llmSpan.SetAttributes(
			attribute.Int64("llm.input_tokens", resp.Usage.InputTokens),
			attribute.Int64("llm.output_tokens", resp.Usage.OutputTokens),
			attribute.Int64("llm.thinking_tokens", thinkingTokens),
		)
		llmSpan.End()

		usage.InputTokens += resp.Usage.InputTokens
		usage.OutputTokens += resp.Usage.OutputTokens
		usage.ThinkingTokens += thinkingTokens
		if reasoningEnabled && metricsInst != nil {
			metricsInst.RecordReasoningCall(ctx, thinkingTokens, reasoningAttrs...)
		}
...
func (p *Provider) runStreamingTurn(
	...
	if err := stream.Err(); err != nil {
		return "", "", nil, usage, classifyAnthropicError(fmt.Errorf("anthropic streaming error: %w", err))
	}
...
func (p *Provider) Complete(ctx context.Context, model, systemPrompt, userMsg string, maxTokens int) (string, error) {
	apiKey := os.Getenv("ANTHROPIC_API_KEY")
	if apiKey == "" {
		return "", agenterrors.NewConfigError(agenterrors.ErrConfigMissing, "ANTHROPIC_API_KEY is not set; required for the anthropic provider", nil)
	}
	if maxTokens <= 0 {
		maxTokens = 4000
	}
	...
	if len(resp.Content) == 0 {
		return "", agenterrors.NewLLMError(agenterrors.ErrLLMProviderError, "provider returned empty response", nil)
	}
...
func wrapAPIError(err error) error {
	...
	if errors.As(err, &apiErr) {
		...
		case 401, 403:
			return agenterrors.NewLLMError(agenterrors.ErrLLMAuthFailed, fmt.Sprintf("%s: %s", queue.PermanentFailure, err), err)
		case 429:
			return agenterrors.NewLLMError(agenterrors.ErrLLMRateLimited, err.Error(), err)
		case 400, 404:
			return agenterrors.NewLLMError(agenterrors.ErrLLMProviderError, fmt.Sprintf("%s: %s", queue.PermanentFailure, err), err)
		}
	}
	return err
}

// classifyAnthropicError wraps an error from the Anthropic API into a structured
// AgentError, preserving the original error chain for errors.Is / errors.As.
func classifyAnthropicError(err error) *agenterrors.AgentError {
	if err == nil {
		return nil
	}

	// Check for timeout / context cancellation first.
	if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
		return agenterrors.NewLLMError(agenterrors.ErrLLMTimeout, fmt.Sprintf("anthropic API error: %s", err), err)
	}

	// Inspect SDK-level error for HTTP status codes.
	var apiErr *anthropicsdk.Error
	if errors.As(err, &apiErr) {
		switch apiErr.StatusCode {
		case 401, 403:
			return agenterrors.NewLLMError(agenterrors.ErrLLMAuthFailed, fmt.Sprintf("anthropic API error: %s", err), err)
		case 429:
			return agenterrors.NewLLMError(agenterrors.ErrLLMRateLimited, fmt.Sprintf("anthropic API error: %s", err), err)
		}
	}

	// Default to generic provider error.
	return agenterrors.NewLLMError(agenterrors.ErrLLMProviderError, fmt.Sprintf("anthropic API error: %s", err), err)
}
...
// buildAnthropicParams assembles anthropic.MessageNewParams from config, messages and tools,
// including reasoning thinking block translation per RFC-0033 DD1 and DD3 clamping.
func buildAnthropicParams(cfg *config.Config, messages []anthropicsdk.MessageParam, tools []anthropicsdk.ToolUnionParam) anthropicsdk.MessageNewParams {
	params := anthropicsdk.MessageNewParams{
		Model:     cfg.Model,
	...
	if cfg.MaxAnswerTokensPerCall > 0 {
		if int64(cfg.MaxAnswerTokensPerCall) < params.MaxTokens {
			params.MaxTokens = int64(cfg.MaxAnswerTokensPerCall)
		}
	}
	...
	// Reasoning on-path: compute effective budget with DD3 clamping.
	effectiveBudget := int64(cfg.ReasoningBudgetTokens)
	if cfg.MaxThinkingTokensPerCall > 0 && int64(cfg.MaxThinkingTokensPerCall) < effectiveBudget {
		effectiveBudget = int64(cfg.MaxThinkingTokensPerCall)
	}
	// Anthropic minimum thinking budget is 1024. Below that, do not enable thinking.
	if effectiveBudget < 1024 {
		return params
	}

	// Anthropic requires max_tokens > thinking.budget_tokens. Ensure room for an answer.
	if params.MaxTokens <= effectiveBudget {
		answerCap := int64(cfg.MaxAnswerTokensPerCall)
		if answerCap <= 0 {
			answerCap = int64(cfg.MaxTokensPerCall)
		}
		params.MaxTokens = effectiveBudget + answerCap
	}
...
// The Anthropic SDK's Usage struct does not expose a dedicated thinking token count,
// so this is a best-effort heuristic. TODO: replace with an SDK-provided counter when
// one becomes available. Returns 0 when no thinking blocks are present.
func countThinkingTokens(_ []anthropicsdk.ContentBlockUnion) int64 {
	// TODO(RFC-0033): The Anthropic SDK does not currently surface a separate
	// thinking token count in Usage; thinking tokens are billed as a subset of
	// OutputTokens. Return 0 until the SDK exposes a dedicated field.
	return 0
}
