...
func (p *Provider) RunTask(
	...
	if apiKey == "" {
		return "", queue.TokenUsage{}, agenterrors.NewConfigError(agenterrors.ErrConfigMissing, "OPENAI_API_KEY is not set; required for the openai provider", nil)
	}
	...
	for {
		...
		if len(resp.Choices) == 0 {
			return "", usage, agenterrors.NewLLMError(agenterrors.ErrLLMProviderError, "openai: empty choices in response", nil)
		}
		choice := resp.Choices[0]

		// Reasoning token accounting: prefer the standard
		// completion_tokens_details.reasoning_tokens (native OpenAI o-series),
		// fall back to estimating from the non-standard "reasoning" message
		// field that Ollama and other OpenAI-compatible servers emit.
		if rt := resp.Usage.CompletionTokensDetails.ReasoningTokens; rt > 0 {
			usage.ThinkingTokens += rt
		} else if rf, ok := choice.Message.JSON.ExtraFields["reasoning"]; ok {
			raw := rf.Raw()
			// Rough estimate: 1 token per 4 chars. Not accurate for billing
			// but gives non-zero observability until the backend reports a
			// real count.
			if len(raw) > 2 { // skip empty string quotes ""
				usage.ThinkingTokens += int64(len(raw) / 4)
			}
		}
...
func (p *Provider) runStreamingTurn(
	...
	if err := stream.Err(); err != nil {
		return "", usage, classifyOpenAIError(fmt.Errorf("openai streaming error: %w", err))
	}
...
func (p *Provider) Embed(ctx context.Context, text string) ([]float32, error) {
	...
	if err != nil {
		return nil, agenterrors.NewLLMError(agenterrors.ErrLLMProviderError, fmt.Sprintf("openai embed: %s", err), err)
	}
	if len(resp.Data) == 0 {
		return nil, agenterrors.NewLLMError(agenterrors.ErrLLMProviderError, "openai embed: empty response", nil)
	}
...
// classifyOpenAIError wraps an error from the OpenAI API into a structured
// AgentError, preserving the original error chain for errors.Is / errors.As.
func classifyOpenAIError(err error) *agenterrors.AgentError {
	if err == nil {
		return nil
	}
	...
	if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
		return agenterrors.NewLLMError(agenterrors.ErrLLMTimeout, fmt.Sprintf("openai API error: %s", err), err)
	}

	// Check for rate limiting via HTTP status 429.
	if strings.Contains(err.Error(), "429") || strings.Contains(strings.ToLower(err.Error()), "rate limit") {
		return agenterrors.NewLLMError(agenterrors.ErrLLMRateLimited, fmt.Sprintf("openai API error: %s", err), err)
	}
	...
	if strings.Contains(err.Error(), "401") || strings.Contains(err.Error(), "403") ||
		...
		return agenterrors.NewLLMError(agenterrors.ErrLLMAuthFailed, fmt.Sprintf("openai API error: %s", err), err)
	}
